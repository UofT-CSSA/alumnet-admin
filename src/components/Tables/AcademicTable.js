...
import { Layout, Menu, Breadcrumb, Typography, Dropdown, Table, Input, Space } from 'antd';
import {
  DesktopOutlined,
  PieChartOutlined,
  FileOutlined,
...
function AcademicTable() {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/AcademicInfo")
  ...
  <Menu>
    <Menu.ItemGroup title="Tables">
      ...
      onClick={() => {
            ...
            }}>Articles Information</Menu.Item>
    </Menu.ItemGroup>
    <SubMenu title="User Information">
      ...
      onClick={() => {
            ...
            }}>Academic Information</Menu.Item>
      <Menu.Item  key="2" icon={<DesktopOutlined />} 
      onClick={() => {
              navigate({ pathname: "/employtable" });
            }}>Employment Information</Menu.Item>
    </SubMenu>
  </Menu>
);
